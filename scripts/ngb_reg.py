...
# Load predictor data
X, dependent = prepare_data(dependent_file_type=file_type, #input_file_path=f'data/gene_presence_absence.Rtab'
                            )
...
logging.basicConfig(filename=f'log_files/ngb/{dateandtime_now()}_{jobid}_{file_type}_{dependent.columns[target_ID]}.log', filemode='w', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
# Step 3: Initialize XGBRegressor and GridSearchCV
ngb = NGBRegressor(random_state=123)
logger.info('defined model')

# Step 4: Fit GridSearchCV
ngb.fit(X_train, y_train)
logger.info('fiited model')

# joblib.dump(ngb, f'/ibex/project/c2205/sara/klebsiella_pneumoniae/model_weights/ngb/{file_type}/{target}')
# logger.info('weights stored')

# Step 6: Make predictions with the best XGBoost model
y_pred_test = ngb.predict(X_test)
y_pred_train = ngb.predict(X_train)
logger.info('prediction done')

preds = ngb.pred_dist(X_train)
y_pred = preds.loc  # Mean of the distribution
y_pred_lower = preds.ppf(0.01)  # Lower bound (1st percentile)
y_pred_upper = preds.ppf(0.99)  # Upper bound (99th percentile)
...
interval_coverage_tr = np.mean((y_train >= y_pred_lower) & (y_train <= y_pred_upper))
interval_width_tr = np.mean(y_pred_upper - y_pred_lower)
...
logger.info(f"Prediction Interval Coverage (PICP):, {interval_coverage_tr}")
logger.info(f"Mean Prediction Interval Width (MPIW):, {interval_width_tr}")
...
interval_coverage_test = np.mean((y_test >= y_pred_test - 1.96 * metrics_test.rmse()) & (y_test <= y_pred_test + 1.96 * metrics_test.rmse()))
interval_width_test = np.mean(2 * 1.96 * metrics_test.rmse())
...
logger.info(f"Prediction Interval Coverage (PICP):, {interval_coverage_test}")
logger.info(f"Mean Prediction Interval Width (MPIW):, {interval_width_test}")
...
# Save results to CSV
test_df.to_csv(f"data/test_danesh/{file_type}/ngb_test_var_{target}.csv", index=False)
train_df.to_csv(f"data/test_danesh/{file_type}/ngb_train_var_{target}.csv", index=False)
