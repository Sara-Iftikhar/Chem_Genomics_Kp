...
logging.basicConfig(filename=f'log_files/quan_reg/{dateandtime_now()}_{jobid}_{file_type}_{dependent.columns[target_ID]}.log', filemode='w', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
joblib.dump(qreg, f'/ibex/project/c2205/sara/klebsiella_pneumoniae/model_weights/quantile/{file_type}/{target}')
logger.info('weights stored')

# Step 6: Make predictions with the best XGBoost model
y_pred_test = qreg.predict(X_test)
y_pred_train = qreg.predict(X_train)
...
interval_coverage_tr = np.mean((y_train >= y_pred_train - 1.96 * metrics_train.rmse()) & (y_train <= y_pred_train + 1.96 * metrics_train.rmse()))
interval_width_tr = np.mean(2 * 1.96 * metrics_train.rmse())
...
# Save results to CSV
test_df.to_csv(f"data/test_danesh/{file_type}/quan_test_var_{target}.csv", index=False)
train_df.to_csv(f"data/test_danesh/{file_type}/quan_train_var_{target}.csv", index=False)
...
with open(f"data/test_danesh/{file_type}/quantile/results_{file_type}.txt", 'a') as fp:
    fp.write(s_string)
