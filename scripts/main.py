...
logging.basicConfig(filename=f'log_files/xgb/new_{dateandtime_now()}_{jobid}_{file_type}_{dependent.columns[target_ID]}.log', filemode='w', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
xgb = XGBRegressor(random_state=313,objective='reg:squarederror', importance_type='gain', max_depth=7, n_estimators=700)
...
# joblib.dump(xgb, f'/ibex/project/c2205/sara/klebsiella_pneumoniae/model_weights/xgb/new_{file_type}/{target}')
...
# Save results to CSV
test_df.to_csv(f"data/test_danesh/{file_type}/new_test_var_{target}.csv", index=False)
train_df.to_csv(f"data/test_danesh/{file_type}/new_train_var_{target}.csv", index=False)
...
with open(f"data/test_danesh/{file_type}/xgb/new_results_{file_type}.txt", 'a') as fp:
    fp.write(s_string)
