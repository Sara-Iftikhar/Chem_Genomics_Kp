...
logging.basicConfig(filename=f'log_files/rf/test_danesh_{dateandtime_now()}_{jobid}_{file_type}_{dependent.columns[target_ID]}.log', filemode='w', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
rf_ref = RandomForestRegressor(random_state=313)
...
joblib.dump(rf_ref, f'model_weights/rf/{file_type}/{target}')
...
# Save results to CSV
test_df.to_csv(f"data/test_danesh/{file_type}/rf_test_var_{target}.csv", index=False)
train_df.to_csv(f"data/test_danesh/{file_type}/rf_train_var_{target}.csv", index=False)
...
with open(f"data/test_danesh/{file_type}/rf/results_{file_type}.txt", 'a') as fp:
    fp.write(s_string)
