...
# Load predictor data
X, dependent = prepare_data(dependent_file_type=file_type, rtab_file_path=f'data/gene_presence_absence.Rtab')
target = dependent.columns[target_ID]

# Load dependent variable data
y = dependent.iloc[:, target_ID]

logging.basicConfig(filename=f'log_files/xgb/test_danesh_{dateandtime_now()}_{jobid}_{file_type}_{dependent.columns[target_ID]}.log', filemode='w', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
# Step 2: Define the parameter grid for hyperparameter tuning for XGBoost
param_grid_xgb = {
    'n_estimators': [10, 50],  # Number of boosting rounds
     'max_depth': [1, 3],  # Maximum tree depth for base learners
...
xgb = XGBRegressor(random_state=313, n_estimators=100, max_depth=7)
...
joblib.dump(xgb, f'model_weights/xgb/{file_type}/{target}')
...
# Save results to CSV
test_df.to_csv(f"data/test_danesh/{file_type}/test_var_{target}.csv", index=False)
train_df.to_csv(f"data/test_danesh/{file_type}/train_var_{target}.csv", index=False)
...
with open(f"data/test_danesh/{file_type}/results_{file_type}.txt", 'a') as fp:
    fp.write(s_string)
